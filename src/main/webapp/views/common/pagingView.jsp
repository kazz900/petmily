...
<div style="text-align:center;">
	<% if(currentPage <= 1){ %>
		＜＜ &nbsp;
	<% }else{ //currentPage > 1 %>
		<a href="/petmily/<%= urlMapping %>?mseq=<%=member.getMemberSeq()%>&page=1">＜＜ &nbsp;</a>
	<% } %>
	<%-- 이전 페이지 그룹으로 이동 --%>
	<% if(((currentPage - 10) < startPage || (currentPage - 10) > 1) && (currentPage - 10) > 0){  //이전그룹이 있다면 %>
		<a href="/petmily/<%= urlMapping %>?mseq=<%=member.getMemberSeq()%>&page=<%= startPage - 10 %>">＜&nbsp;</a> &nbsp;
	<% }else{ //이전그룹이 없다면 %>
		＜ &nbsp;
	<% } %>
	...
	<%     }else{ %>
			<a href="/petmily/<%= urlMapping %>?mseq=<%=member.getMemberSeq()%>&page=<%= p %>" style="font-size:5px;">&nbsp;<%= p %>&nbsp;</a>
	<% }} %>
	...
	<% }else{ //currentPage < maxPage  %>
		<a href="/petmily/<%= urlMapping %>?mseq=<%=member.getMemberSeq()%>&page=<%= maxPage %>">＞＞</a> &nbsp;
	<% } %>
</div>
